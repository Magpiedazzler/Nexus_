...
module.exports.showAllApps=async(req,res)=>{
    try{
        ...
        return res.json({
            status:false,
            message:"Unable to find users's application"
        });
    }
};
...
module.exports.FetchGames = async (req, res) => {
  ...
  module.exports.review=async(req,res)=>{
    try{
      const appId=req.params.appId
      const reviewData=await rateModel.find({appId})
      if(reviewData){
        return res.json({message:"Success", status:true,reviewData})
      }
      return res.json({message:"Unable to fetch rating", status:false})
    }catch(error){
      console.log(error);
      return res.json({ message: "Internal server error", status: false });
    }
